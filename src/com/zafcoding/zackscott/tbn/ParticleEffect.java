...
import com.zafcoding.zackscott.tbn.ParticleEffect.ParticleData;
import com.zafcoding.zackscott.tbn.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	...
	WATER_BUBBLE("bubble", 4, -1, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_WATER),
	/**
	 ...
	 * </ul>
	 */
	ITEM_CRACK("iconcrack", 36, -1, ParticleProperty.DIRECTIONAL,
	 ...
	 * </ul>
	 */
	BLOCK_CRACK("blockcrack", 37, -1, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_DATA),
	/**
	 ...
	 * </ul>
	 */
	BLOCK_DUST("blockdust", 38, 7, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_DATA),
	/**
	 ...
	 */
	private static boolean isDataCorrect(ParticleEffect effect,
			ParticleData data) {
	 ...
	 */
	private static boolean isColorCorrect(ParticleEffect effect,
			ParticleColor color) {
	...
	public void display(float offsetX, float offsetY, float offsetZ,
		...
		new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount,
				range > 256, null).sendTo(center, range);
	}
	 ...
	 */
	public void display(float offsetX, float offsetY, float offsetZ,
			float speed, int amount, Location center, Player... players)
			throws ParticleVersionException, ParticleDataException,
			IllegalArgumentException {
		display(offsetX, offsetY, offsetZ, speed, amount, center,
				Arrays.asList(players));
	}

	public void displayz(float offsetX, float offsetY, float offsetZ,
			float speed, int amount, Location center, Object[] player)
			throws ParticleVersionException, ParticleDataException,
			IllegalArgumentException {
		ArrayList<Player> temp1010 = new ArrayList<Player>();
		for (Object ob : player) {
			if (ob instanceof Player) {
				temp1010.add((Player) ob);
			}
		}
		display(offsetX, offsetY, offsetZ, speed, amount, center, temp1010);
	}
	...
	public void display(Vector direction, float speed, Location center,
			double range) throws ParticleVersionException,
		...
		new ParticlePacket(this, direction, speed, range > 256, null).sendTo(
				center, range);
	}
	 ...
	 */
	public void display(Vector direction, float speed, Location center,
			List<Player> players) throws ParticleVersionException,
			ParticleDataException, IllegalArgumentException {
		if (!isSupported()) {
			throw new ParticleVersionException(
					"This particle effect is not supported by your server version");
		...
		new ParticlePacket(this, direction, speed, isLongDistance(center,
				players), null).sendTo(center, players);
	}
	 ...
	 */
	public void display(Vector direction, float speed, Location center,
			Player... players) throws ParticleVersionException,
			ParticleDataException, IllegalArgumentException {
	...
	public void display(ParticleColor color, Location center, double range)
		...
		if (!hasProperty(ParticleProperty.COLORABLE)) {
			throw new ParticleColorException(
					"This particle effect is not colorable");
		}
	 ...
	 * @see ParticlePacket#sendTo(Location, List)
	 */
	public void display(ParticleColor color, Location center,
			List<Player> players) throws ParticleVersionException,
		...
		new ParticlePacket(this, color, isLongDistance(center, players))
	...
	public void display(ParticleData data, float offsetX, float offsetY,
		...
		new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount,
				range > 256, data).sendTo(center, range);
	}
	...
	public void display(ParticleData data, Vector direction, float speed,
			Location center, double range) throws ParticleVersionException,
			ParticleDataException {
		if (!isSupported()) {
			throw new ParticleVersionException(
					"This particle effect is not supported by your server version");
		...
		new ParticlePacket(this, direction, speed, range > 256, data).sendTo(
				center, range);
	}
	 ...
	 * @since 1.6
	 */
	private static final class ParticleVersionException extends
			RuntimeException {
		private static final long serialVersionUID = 3203085387160737484L;
	...
	public static final class ParticlePacket {
		 ...
		 * @see #initialize()
		 */
		public ParticlePacket(ParticleEffect effect, float offsetX,
				float offsetY, float offsetZ, float speed, int amount,
				boolean longDistance, ParticleData data)
				throws IllegalArgumentException {
			initialize();
			if (speed < 0) {
		 ...
		 *      boolean, ParticleData)
		 */
		public ParticlePacket(ParticleEffect effect, Vector direction,
				float speed, boolean longDistance, ParticleData data)
				throws IllegalArgumentException {
		 ...
		 *      boolean, ParticleData)
		 */
		public ParticlePacket(ParticleEffect effect, ParticleColor color,
				boolean longDistance) {
			this(effect, color.getValueX(), color.getValueY(), color
					.getValueZ(), 1, 0, longDistance, null);
		}
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				version = Integer.parseInt(Character.toString(PackageType
						.getServerVersion().charAt(3)));
				if (version > 7) {
					enumParticle = PackageType.MINECRAFT_SERVER
				...
				playerConnection = ReflectionUtils
						.getField("EntityPlayer", PackageType.MINECRAFT_SERVER,
		...
		private void initializePacket(Location center)
			...
			try {
				...
				if (version < 8) {
					...
					ReflectionUtils.setValue(packet, true, "a",
							enumParticle.getEnumConstants()[effect.getId()]);
